...
evalSKI :: SKI a -> a
evalSKI (Ap l r) = evalSKI l (evalSKI r)
evalSKI S = \fab fa a -> fab a (fa a)
evalSKI K = const
evalSKI I = id
...
prettyPrintSKI s = iter s ""
  where
    iter :: SKI a -> String -> String
    iter (Ap l r) = ('(' :) . iter l . (' ' :) . iter r . (')' :)
    iter S = ('S' :)
    iter K = ('K' :)
    iter I = ('I' :)
...
skiComp :: SKI (b -> c) -> SKI (a -> b) -> SKI (a -> c)
skiComp l r = comp `Ap` l `Ap` r

flip' :: SKI ((a -> b -> c) -> (b -> a -> c))
flip' = (rev `Ap` K) `skiComp` S `skiComp` K `skiComp` S

rotr :: SKI (a -> (c -> a -> b) -> c -> b)
rotr = S `skiComp` K `skiComp` rev

rotv :: SKI (a -> b -> (a -> b -> c) -> c)
rotv = (rotr `Ap` rotr `Ap` comp) `skiComp` (comp `skiComp` rev)
...
and' = S `Ap` (K `Ap` (rev `Ap` false)) `Ap` flip'

or' :: SKI (Bool' (Bool' a) -> Bool' a -> Bool' a)
or' = rev `Ap` true

xor' :: SKI (Bool' (Bool' a) -> Bool' a -> Bool' a)
xor' = (flip' `Ap` S `Ap` not') `skiComp` flip'

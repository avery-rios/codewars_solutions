...
evalJoin :: Database -> Join -> V.Vector JoinedRow -> V.Vector JoinedRow
            ...
            V.mapMaybe
              ( \r ->
                  let jr1 = HM.insertWith HM.union tName r jr
                   in if evalTest vt jr1 then Just jr1 else Nothing
              )
              rs
        )
        jrs
...
execute :: Database -> Query -> V.Vector ResultRow
execute db q@Query {qFrom = From tbl js} =
  V.concatMap
    ( \r ->
        V.mapMaybe
          (evalSelect (qSelect q))
          ( let jrs =
                  foldl'
                    (\jr j -> evalJoin db j jr)
                    (V.singleton (HM.singleton tbl r))
                    js
             in case qWhere q of
                  Just vt -> V.filter (evalTest vt) jrs
                  Nothing -> jrs
          )
    )
    (HM.lookupDefault V.empty tbl db)

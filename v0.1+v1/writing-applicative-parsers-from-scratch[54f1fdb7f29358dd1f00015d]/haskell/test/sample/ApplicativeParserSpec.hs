...
spec = do
  ...
  describe "other combinators" $ do
    ...
    it "stringP shouldn't parse string with different input" $
      property $
  ...
  where
    {-describe "do not cheat" $ do
        it "do not use Control.Applicative" $ solutionShouldHideAll [Module "Control.Applicative"]
        it "do not use Data.Functor" $ solutionShouldHideAll [Module "Data.Functor"]
        it "do not use fmap" $ solutionShouldHideAll $ map (FromModule "Prelude") ["fmap"]-}
